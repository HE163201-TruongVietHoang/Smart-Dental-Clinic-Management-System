...
async function getOrCreateConversationService(participant1Id, participant2Id) {
  return await conversationAccess.findOrCreateConversation(
    participant1Id,
    participant2Id
  );
}
...
async function sendMessageService(
  ...
  return await messageAccess.addMessage(
    conversationId,
    senderId,
    content,
    messageType
...
async function getMessagesService(conversationId, limit = 50, offset = 0) {
  return await messageAccess.getMessagesByConversation(
    conversationId,
    limit,
    offset
  );
...
async function transferConversationService(conversationId, newReceptionistId) {
  return await conversationAccess.transferConversation(
    conversationId,
    newReceptionistId
  );
}
